...
pub struct OutStream {
    pub(crate) outstream: *mut raw::SoundIoOutStream,

    pub(crate) write_callback: Box<dyn FnMut(&mut OutStreamWriter)>,
    pub(crate) underflow_callback: Option<Box<dyn FnMut()>>,
    pub(crate) error_callback: Option<Box<dyn FnMut(Error)>>,

    pub(crate) parent_device: Rc<Device>,
}
...
impl OutStream {
    ...
    pub fn start(&mut self) -> Result<()> {
        match unsafe { raw::soundio_outstream_start(self.outstream) } {
            0 => Ok(()),
            x => Err(x.into()),
        }
    }
    ...
    pub fn clear_buffer(&mut self) -> Result<()> {
        match unsafe { raw::soundio_outstream_clear_buffer(self.outstream) } {
            0 => Ok(()),
            e => Err(e.into()),
        }
    }
    ...
    pub fn pause(&mut self, pause: bool) -> Result<()> {
        match unsafe { raw::soundio_outstream_pause(self.outstream, pause as i8) } {
            0 => Ok(()),
            e => Err(e.into()),
        }
    }
    ...
    pub fn format(&self) -> Format {
        unsafe { (*self.outstream).format.into() }
    }
    ...
    pub fn name(&self) -> String {
        unsafe { utf8_to_string((*self.outstream).name) }
    }
    ...
    pub fn bytes_per_frame(&self) -> i32 {
        unsafe { (*self.outstream).bytes_per_frame as _ }
    }

    /// The number of bytes in a sample, e.g. 3 for `i24`.
    pub fn bytes_per_sample(&self) -> i32 {
        unsafe { (*self.outstream).bytes_per_sample as _ }
    }
}
...
impl<'a> OutStreamWriter<'a> {
    ...
    pub fn end_write(&mut self) {
        if self.write_started {
            unsafe {
                match raw::soundio_outstream_end_write(self.outstream.outstream) {
                    0 => {
    ...
    pub fn channel_count(&self) -> usize {
        unsafe { (*self.outstream.outstream).layout.channel_count as _ }
    }
    ...
    pub fn get_latency(&mut self) -> Result<f64> {
        ...
        match unsafe {
            raw::soundio_outstream_get_latency(self.outstream.outstream, &mut x as *mut c_double)
        } {
            0 => Ok(x),
            e => Err(e.into()),
        }
    ...
    pub fn set_sample<T: Sample>(&mut self, channel: usize, frame: usize, sample: T) {
        ...
        unsafe {
            ...
            match (*self.outstream.outstream).format {
